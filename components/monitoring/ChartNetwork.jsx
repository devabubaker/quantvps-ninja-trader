    ...
    if (active) {
      return (
        <div className='custom-tooltip bg-white rounded-[6px] border p-2 shadow-sm'>
          ...
          <p className='label text-[14px] text-[#8884d8]'>{`Inbound: ${(payload[0]?.value / 1000).toFixed(2)} Kbps`}</p>
          <p className='label text-[14px] text-[#82ca9d]'>{`Outbound: ${(payload[1]?.value / 1000).toFixed(2)} bps`}</p>
        </div>
      )
    }
